...
void init(int level)
{
    ...
    printf("\n");
    printf("第一次点击只能左键单击也就是op是1\n");
    scanf("%d %d %d", &initOp, &initR, &initC);//确定初始点击位置
    ...
    while (placed_mines < mines){
        ...
        if (rand_row == initR-1 && rand_col == initC-1) {//初始点击位置设置不能设置为雷
            continue;  // 跳过指定位置
        }
        if (mine[rand_row][rand_col] == 0) {
            mine[rand_row][rand_col] = 1;
            placed_mines++;
        }
    }

//对初始点击的回应
    open(initR-1,initC-1);
    opcount++; // 操作数+1
    printf("Game in progress\n");
    printf("%d %d\n", opcount, mines);
    print_board();
